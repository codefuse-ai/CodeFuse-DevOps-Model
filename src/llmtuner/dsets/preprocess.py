...
def preprocess_dataset(
    ...
    def preprocess_pretrain_dataset(examples: Dict[str, List[Any]]) -> Dict[str, Any]:
        # build grouped texts with format `X1 eos X2 eos X3 ...` (without <eos>)
        ...
        # add eos
        examples['prompt'] = [i + tokenizer.eos_token for i in examples['prompt']]
    ...
    def preprocess_pretrain_dataset_pretokenized(examples: Dict[str, List[Any]]) -> Dict[str, Any]:
        # build grouped texts with format `X1 eos X2 eos X3 ...` (without <eos>)
        if isinstance(getattr(tokenizer, "tokenizer", None), tiktoken.Encoding): # for tiktoken tokenizer (Qwen)
            kwargs = dict(allowed_special="all")
        else:
    ...
    def print_supervised_dataset_example(example):
        print("input_ids:{}".format(example["input_ids"]))
        print("inputs:{}".format(tokenizer.decode(example["input_ids"], skip_special_tokens=False)))
        print("label_ids:{}".format(example["labels"]))
        print("labels:{}".format(tokenizer.decode([
    ...
    # If dataset is already been preprocessed, skip and return
    if data_args.preprocessed_dataset_path:
        d = iter(dataset)
        print_unsupervised_dataset_example(next(d))
        return dataset
    ...
    with training_args.main_process_first(desc="dataset map pre-processing"):
        ...
        # saving dataset
        if data_args.save_dataset_path and training_args.local_rank == 0:
            logger.info('[rank={}]Saving dataset to {}'.format(training_args.local_rank, data_args.save_dataset_path))
            dataset.save_to_disk(data_args.save_dataset_path)

        if training_args.local_rank == 0:
            d = iter(dataset)
            print_function(next(d))
        return dataset
