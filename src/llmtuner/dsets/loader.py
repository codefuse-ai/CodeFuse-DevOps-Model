...
def get_dataset(
    ...
    if data_args.preprocessed_dataset_path:
        logger.info("Loading preprocessed dataset {}...".format(data_args.preprocessed_dataset_path))
        dataset = load_from_disk(data_args.preprocessed_dataset_path)
        return dataset
    ...
    for dataset_attr in data_args.dataset_list:
        ...
        logger.info('data_path={}, data_files={}, split={}, cache_dir={}, streaming={}, use_auth_token={}'.format(data_path, 
                                                                                                                  data_files, 
                                                                                                                  data_args.split, 
        ...
        for column_name in ["prompt", "query", "response", "history"]: # align datasets
            if getattr(dataset_attr, column_name) and getattr(dataset_attr, column_name) != column_name and dataset.column_names:
...
if __name__ == '__main__':
    dataset = load_dataset(
            'json',
            data_files=['/mnt/llm/devopspal/data/sst_data/devopspal_corpus.json'],
            split='train',
            cache_dir=None,
            streaming=False,
            use_auth_token=None
        )
    # data_path=json, data_files=['/mnt/llm/devopspal/data/sst_data/devopspal_corpus.json'], split=train, cache_dir=None, streaming=False, use_auth_token=False